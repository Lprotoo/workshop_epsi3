...
class AIAgent:
    ...
    EXERCISE_KEYWORDS = {
        "stress": ["stress", "anxiete", "angoisse", "nervosite", "agitation", "panique"],
        "sommeil": ["insomnie", "fatigue", "reveils", "sommeil"],
        "energie": ["concentration", "brouillard", "vigilance", "epuise"],
        "musculaire": ["courbature", "douleur", "tension", "musculaire"]
    }
    ...
    def load_exercises(self) -> Dict[str, Any]:
        """Charge la bibliotheque d'exercices depuis data/exercises.json"""
        path = os.path.join(os.path.dirname(__file__), "..", "data", "exercises.json")
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)

    def suggest_exercise_plan(self, detected_signs: list, stress_level: int = 0) -> Dict[str, Any]:
        """Propose un mini-programme sportif base sur les signes detectes par l'IA."""
        exercises = self.load_exercises()
        signs_text = " ".join(detected_signs).lower() if detected_signs else ""

        categories_matched = []
        for category, keywords in self.EXERCISE_KEYWORDS.items():
            if any(kw in signs_text for kw in keywords):
                categories_matched.append(category)

        if stress_level >= 6 and "stress" not in categories_matched:
            categories_matched.append("stress")
        ...
        return {"triggered": True, "plan": plan}
        ...
        try:
            ...
            if 'choices' in result and len(result['choices']) > 0:
                ...
                try:
                    ...
                    # Ajout du programme sportif base sur les signes detectes
                    analysis['exercise_plan'] = self.suggest_exercise_plan(
                        analysis.get('detected_signs', []),
                        stress_level
                    )
